...
(function(window, document) {
  ...
  function spinWheel() {
    ...
    // Store timeout ID so we can clear it if STOP is pressed
    let spinTimeoutId;
    ...
    currentRotation = total;
      ...
      // Play result sound when confetti starts dropping
      setTimeout(() => {
    ...
    // Store the timeout ID globally so stopWheel can access it
    window.currentSpinTimeoutId = spinTimeoutId;
  }
  ...
  function stopWheel() {
    ...
    // Clear the natural spin timeout to prevent double result sound
    if (window.currentSpinTimeoutId) {
      clearTimeout(window.currentSpinTimeoutId);
      window.currentSpinTimeoutId = null;
    }
